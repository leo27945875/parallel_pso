...
"""
    ...
    cpu_time_all, cpu_time_main = exec_timer(cpu_timer, "CPU", n_test, is_all)
...
"""
    ...
    cuda_time_all, cuda_time_main = exec_timer(cuda_timer, "CUDA", n_test, is_all)
...
def main():
    ...
    parser.add_argument("-r", "--r_ord" , type=int, default=7     , help="The maximal order among the testing dimensions. (dim = 2 ** <order> - 1)")
    ...
    for i, o in enumerate(range(args.l_ord, args.r_ord + 1)):
        dims[i] = 2 ** o - 1
        print(f"\n >> Iter {i + 1}: (dim={dims[i]}, num={dims[i] * 2**5})")
        if args.device == "cpu":
            cpu_time_all, cpu_time_main = perf_cpu(dim=dims[i], n_iter=args.n_iter, n_test=args.n_test, is_all=args.all)
            cpu_time_alls [i] = cpu_time_all  * 1000. if cpu_time_all is not None else None
            cpu_time_mains[i] = cpu_time_main * 1000.
        elif args.device == "gpu":
            gpu_time_all, gpu_time_main = perf_gpu(dim=dims[i], n_iter=args.n_iter, n_test=args.n_test, is_all=args.all)
            gpu_time_alls [i] = gpu_time_all  * 1000. if cpu_time_all is not None else None
