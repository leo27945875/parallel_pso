...
class PSO_Pthread:
    def __init__(
        ...
        pthreadPSO.set_num_threads(n_thread)
        ...
        pthreadPSO.calc_fitness_vals(self.xs, self.x_fits)
    ...
    def step(self, i: int, verbose: int = 1, plt_line: Artist | None = None) -> Iterable[Artist] | None:
        ...
        pthreadPSO.update_velocities_and_positions(self.xs, self.vs, self.local_best_xs, self.global_best_x, self.w, self.c0, self.c1, self.x_min, self.x_max, -self.v_max, self.v_max)
        pthreadPSO.calc_fitness_vals(self.xs, self.x_fits)
        pthreadPSO.update_best_values(self.xs, self.local_best_xs, self.x_fits, self.local_best_fits, self.global_best_x, self.global_best_fit)
